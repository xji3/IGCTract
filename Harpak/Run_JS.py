...
class Run_PSJS_Harpak_all:
    ...
    def objective_wo_gradient(self, display, x):
        ...
        f = 0.0
        for psjsgeneconv in self.psjsgeneconv_list:
            f_incr = psjsgeneconv.objective_wo_gradient(display = display, x = x)
            f += f_incr
        return f
...
class Run_JS_Harpak_all:
    ...
    def objective_wo_gradient(self, display, x):
        ...
        f = 0.0
        for jsgeneconv in self.jsgeneconv_list:
            f_incr = jsgeneconv.objective_wo_gradient(display = display, x = x)
            f += f_incr
        return f

    def get_Godambe_matrix(self, x, gradient_file_list, hessian_file_list):
        for i, jsgeneconv in enumerate(self.jsgeneconv_list):
            jsgeneconv.get_Godambe_matrix(x, gradient_file_list[i], hessian_file_list[i])

    def get_summary(self, summary_file_list):
    	for i, jsgeneconv in enumerate(self.jsgeneconv_list):
    		jsgeneconv.get_individual_summary(summary_file_list[i])
