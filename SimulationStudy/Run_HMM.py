...
def main(args):
    ...
    for sim_num in range(1, 101):
        alignment_file = './Tract_' + str(geo) + '_' + model +'/sim_' + str(sim_num) + '/YDR418W_YEL054C_sim_' + str(sim_num) + '.fasta'
        newicktree = './YeastTree.newick'
        Force = None
        seq_index_file = '../MafftAlignment/YDR418W_YEL054C/YDR418W_YEL054C_seq_index.txt'

        save_path = './save/Tract_' + str(geo) + '_' + model +'/sim_' + str(sim_num)
        if not os.path.isdir('./save/Tract_' + str(geo)+'_' + model):
            os.mkdir('./save/Tract_' + str(geo)+'_' + model)
        if not os.path.isdir('./save/Tract_' + str(geo) + '_' + model +'/sim_' + str(sim_num)):
            os.mkdir('./save/Tract_' + str(geo) + '_' + model +'/sim_' + str(sim_num))

        if rate_variation:
            save_name = './save/Tract_' + str(geo) + '_' + model +'/sim_' + str(sim_num) + '/YDR418W_YEL054C_Ind_' + model +'_rv_IGC_sim_' + str(sim_num) + '_save.txt'
        else:
            save_name = './save/Tract_' + str(geo) + '_' + model +'/sim_' + str(sim_num) + '/YDR418W_YEL054C_Ind_' + model +'_IGC_sim_' + str(sim_num) + '_save.txt'
    ...
    ###### Now get Ind MG94+IS-IGC+HMM estimates
        summary_path = './summary/Tract_' + str(geo) + '_' + model +'/sim_' + str(sim_num) +'/'
        if not os.path.isdir('./summary/Tract_' + str(geo)+'_' + model):
            os.mkdir('./summary/Tract_' + str(geo)+'_' + model)
        ...
        if rate_variation:
            ...
            plot_file = './plot/Tract_' + str(geo) + '_' + model + '/sim_' + str(sim_num) + '/HMM_' + '_'.join(paralog)+ '_' + model + '_rv_lnL_sim_' + str(sim_num) + '_1D_surface.txt'
        else:
            summary_file_1D = summary_path + 'HMM_' + '_'.join(paralog) + '_' + model + '_nonclock_sim_' + str(sim_num) + '_1D_summary.txt'
            plot_file = './plot/Tract_' + str(geo) + '_' + model + '/sim_' + str(sim_num) + '/HMM_' + '_'.join(paralog)+ '_' + model + '_lnL_sim_' + str(sim_num) + '_1D_surface.txt'

        
        # Plot lnL surface in IS-IGC+HMM model
        log_p_list = np.log(3.0/np.array(range(3, 1001)))
        
        if not os.path.isdir('./plot/Tract_' + str(geo) + '_' + model):
            os.mkdir('./plot/Tract_' + str(geo) + '_' + model)
        if not os.path.isdir('./plot/Tract_' + str(geo) + '_' + model + '/sim_' + str(sim_num)):
            os.mkdir('./plot/Tract_' + str(geo) + '_' + model + '/sim_' + str(sim_num))
